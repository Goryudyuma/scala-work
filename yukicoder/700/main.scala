...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(if (recursive(sc, sc.nextLong + 1)) ("YES") else ("NO"))
  }

  def recursive(sc: => Scanner, N: Long): Boolean = {
    if (N == 0) (false) else if (sc.next.contains("LOVE"))(true) else (recursive(sc, N - 1))
  }
