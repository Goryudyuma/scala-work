...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(
      Array.fill(sc.nextInt())((sc.nextLong(), sc.nextLong(), sc.nextLong()))
        .map(calc)
        .mkString("\n")
    )
  }
  ...
  def calc(input: (Long, Long, Long)): Long = {
    input match {
      case (a, b, c) => {
        var ans: Long = Long.MaxValue
        if (b >= 3 && (a >= 2 || c >= 2)) {
          ans = Math.min(ans, Math.max(a - (b - 1), 0) + Math.max(c - Math.min(a - 1, b - 2), 0))
          ans = Math.min(ans, Math.max(a - Math.min(c - 1, b - 2), 0) + Math.max(c - (b - 1), 0))
        }
        //        println(ans)
        if (b >= 2 && (a >= b && c >= b + 1)) {
          ans = Math.min(ans, 1 + Math.max(0, a - (c - 1)))
        }
        //        println(ans)
        if (b >= 2 && (a >= b + 1 && c >= b)) {
          ans = Math.min(ans, 1 + Math.max(0, c - (a - 1)))
        }
        //        println(ans)
        if (a >= 2 && c >= 2)
          if (a != c)
            ans = Math.min(ans, Math.max(0, b - (Math.min(a, c) - 1)))
          else if (a >= 3 && c >= 3)
            ans = Math.min(ans, Math.max(0, b - (Math.min(a, c) - 2)) + 1)
        //        println(ans)
