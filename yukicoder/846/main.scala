...
object Main {
  def solve(sc: => Scanner): Unit = {
    val P, Q, R, A, B, C = sc.nextLong()
    val X = Math.max(Math.max((A - 1) * P + 1, (A + B - 1) * Q + 1), (A + B + C - 1) * R + 1)
    val Y = Math.min(Math.min(A * P, (A + B) * Q), (A + B + C) * R)
    if (X <= Y) println(X + " " + Y) else println(-1)
  }
