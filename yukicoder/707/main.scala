...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, M = sc.nextInt()
    val A = Array.fill(N)(sc.next().toCharArray.zipWithIndex.filter(x => x._1 == '1').map(x => x._2)).zipWithIndex.map(x => x._1.map(y => ((x._2, y)))).flatten
    println((-1 to N).map(i => (-1 to M).map(j => if (i == -1 || j == -1 || i == N || j == M) (A.map(k => math.sqrt(math.pow(i - k._1, 2) + math.pow(j - k._2, 2))).sum) else (1L << 60)).min).min)
