...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    val A = (0 until S.length).map(i => Util.getPermutation().map(x => x.toInt).takeWhile(j => i - j >= 0 && i + j < S.size && S(i - j) == S(i + j)).length * 2 - 1).max
    val B = (0 until S.length - 1).map(i => Util.getPermutation().map(x => x.toInt).takeWhile(j => i - j + 1 >= 0 && i + j < S.size && S(i - j + 1) == S(i + j)).length * 2 - 2).max
    val ans = math.max(A, B)
    println(if (S.length == 2) (1) else if (ans == S.length) (ans - 2) else (ans))
  }
