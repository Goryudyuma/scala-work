...
object Main {
  def solve(sc: => Scanner): Unit = {
    println("012345678910111213141516171819202122232425262728293031323334353637383940414243444546474849505152535455".charAt(sc.nextInt()))
  }
  ...
  def recursive(S: String): Long = {
    if (S.length() == 0) (0) else (recursive(S.tail.dropWhile(c => c > S.head)) + 1)
  }
