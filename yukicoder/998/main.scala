...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(if (Array.fill(4)(sc.nextLong()).sorted.sliding(2).forall(x => x(0) + 1 == x(1))) "Yes" else "No")
  }
  ...
  def recursive(A: Long, B: Long, ans: Long): Long = {
    if (A == 0) ans else recursive(A / B, B, ans + 1)
  }
