...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    var now: Array[Long] = Array.fill(K + 1)(0L)
    now(0) = 1L
    val minV = (0 until N).map(_ => {
      val next = Array.fill(K + 1)(0L)
      val ret = (0 until M).map(_ => {
        val A = sc.nextInt()
        (0 to K - A).foreach(i => {
          next(i + A) += now(i)
        })
    ...
    println(if (minV > K) -1 else K - now.zipWithIndex.filter(x => x._1 > 0).map(x => x._2).max)
  }
