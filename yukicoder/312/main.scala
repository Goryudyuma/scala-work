...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    if (N == 4) {
      ...
      println(math.min(recursive(N, Util.getPermutation(3)), N / 2))
    } else {
      println(recursive(N, Util.getPermutation(3)))
    }
  }

  def recursive(N: => Long, S: Stream[Long]): Long = {
    if (N % S.head == 0) (S.head) else if (S.head * S.head > N) (N) else (recursive(N, S.tail))
  }
