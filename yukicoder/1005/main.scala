...
object Main {
  def solve(sc: => Scanner): Unit = {
    val S, T = sc.next()
    println(
      if (T.length == 1 && S.contains(T)) -1
      else {
        val Snext = recursive(S, T)
        if (Snext.contains(T)) -1
        else Snext.count(_ == '.')
      }
    )
  }
  ...
  def recursive(S: String, T: String): String = {
    val Snext = S.replaceFirst(T, T.take(T.length - 1) + "." + T.last)
    if (Snext == S) S else recursive(Snext, T)
  }
