...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A = Array.fill(5)(sc.nextLong()).reverse
    val F = getFibonacci().drop(1).dropWhile(i => i < A.head).take(6)
    println(
      Math.max(
        A.zip(F).takeWhile(x => x._1 == x._2).length,
        A.zip(F.drop(1)).takeWhile(x => x._1 == x._2).length)
    )
  }
