...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A, B = sc.nextLong()
    println(if (math.abs(A) >= 100 && math.abs(B) >= 100 && check(A) && check(B)) (A * B / 10) else {
      val C = A * B
      if (-99999999 <= C && C <= 99999999) (C) else ("E")
    })
  }

  def recursive(A: => Array[Long], N: Long): Long = {
    if (A.size == 0) (0) else (if (A.head == N) (1 + recursive(A.tail, N - 1)) else (recursive(A.tail, N)))
  }
