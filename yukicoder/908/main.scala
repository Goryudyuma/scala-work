...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A = sc.nextLine().zipWithIndex
    println(if (A.filter(x => x._2 % 2 == 1).forall(x => x._1 == ' ')
      && A.filter(x => x._2 % 2 == 0).forall(x => x._1 != ' ')) "Yes" else "No")
  }

  def recursive(i: Long, j: Long, k: Long): Long = {
    if (i <= j) 1L else recursive(i, j * k, k) + 1
  }
