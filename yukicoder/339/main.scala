...
object Main {
  ...
  def solve(sc: => Scanner): Unit = {
    println(100 / Array.fill(sc.nextInt)(sc.nextInt).foldLeft(Util.getPermutation().take(100).foldLeft(1L)((a, b) => a * b))((a, b) => gcd(a, b)))
  }

  def recursive(X: Long): (Long, Long) = {
    if (X == 0) ((0, 0)) else {
      val ret = recursive(X / 2)
      (math.max(ret._1, X - ret._2), ret._2 + X)
    }
  }

  def recursive2(X: Long): String = {
    if (X == 0) ("") else (recursive2((X - 1) / 2) + (if (X % 2 == 0) ("R") else ("L")))
  }
