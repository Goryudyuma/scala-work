...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    (2 to N).foreach(i => if (A(i) == 0) (1 to N / i).foreach(j => A(j * i) += 1))
    println(A.drop(2).count(_ >= K))
  }

  val mod: Long = (1e9 + 7).toLong

  @scala.annotation.tailrec
  def recursive(N: Long, i: Long, A: Long, B: Long, C: Long): Long = {
    if (i == N + 1) A
    else {
      recursive(N, i + 1, i * C % mod, C, (B + i * C) % mod)
    }
  }
