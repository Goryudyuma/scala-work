...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A, B = Array.fill(3)(sc.nextLong).sorted
    println(2 * (A(0) * (B(1) + B(2)) + A(1) * (B(0) + B(2)) + A(2) * (B(0) + B(1))))
  }

  def recursive(sc: => Scanner, q: Long, s: Seq[Long]): Unit = {
    if (q != 0) {
      val p = sc.nextInt
      if (p == 0) {
        recursive(sc, q - 1, s :+ sc.nextLong)
      } else if (p == 1) {
        println(s(sc.nextInt))
        recursive(sc, q - 1, s)
      } else {
        recursive(sc, q - 1, s.reverse.tail.reverse)
      }
    }
  }
