...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    println(N match {
      case 6 => "Kanzensu!"
      case 28 => "Kanzensu!"
      case i if (2L to 8L).map(x => x * x).contains(i) => "Heihosu!"
      case i if (2L to 8L).map(x => x * x * x).contains(i) => "Ripposu!"
      case i if Util.getPrimeList.takeWhile(x => x <= 64L).contains(i) => "Sosu!"
      case i => i
    })
  }
