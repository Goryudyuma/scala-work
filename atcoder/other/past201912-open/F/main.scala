...
object Main {
  def solve(sc: => Scanner): Unit = {
    val S = sc.next()
    println(
      S.foldLeft((Array[String](), new StringBuilder)) {
        case ((a, b), c) if b.isEmpty => (a, b.append(c))
        case ((a, b), c) if c.isUpper => (a :+ b.append(c).toString(), {
          b.clear(); b
        })
        case ((a, b), c) => (a, b.append(c))
      }._1.sortBy(_.toUpperCase).mkString("")
    )
