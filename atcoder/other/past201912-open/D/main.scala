...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    val A = Array.fill(N)(sc.nextLong()).groupBy(identity)
    val ans = (1 to N).find(i => !A.contains(i))
    println(
      ans match {
        case Some(i) => A.filter(x => x._2.length != 1).head._1 + " " + i
        case None => "Correct"
      }
    )
  }
  ...
  def recursive(coin: Array[Long], money: Long, ans: Long = 0): Long = {
    if (coin.length == 0) ans
    else recursive(coin.tail, money % coin.head, ans + money / coin.head)
  }
