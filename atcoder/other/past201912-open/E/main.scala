...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    (0 until Q).foreach(_ => {
      sc.nextInt() match {
        case 1 => A(sc.nextInt() - 1) += sc.nextInt() - 1
        case 2 => {
          val a = sc.nextInt() - 1
          A(a) ++= (0 until N).filter(i => A(i).contains(a))
        }
        case _ => {
          val a = sc.nextInt() - 1
          A(a) ++= A(a).flatMap(i => A(i)) - a
        }
      }
    })
    println(A.map(x => (0 until N).map(i => x.contains(i)).map(if (_) 'Y' else 'N').mkString("")).mkString("\n"))
  }
