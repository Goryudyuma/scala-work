...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(sc.next().scanLeft('?')((a, b) => if (a == 'P' && b == '?') 'D' else b).tail
      .reverse.scanLeft('P')((a, b) => if (b != '?') b else if (a == 'P') 'D' else 'P').tail.reverse.mkString(""))
  }

  @scala.annotation.tailrec
  def recursive(A: Array[Long], ans: Long = 0, prev: Long = 0): Long = {
    println(ans, prev)
    if (A.length == 0) ans
    else recursive(A.tail, ans + A.head + (prev + 1) / 2, A.head + (prev + 1) / 2)
  }
