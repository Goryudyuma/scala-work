...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A = Array.fill(3)((sc.nextLong(), sc.nextLong()))
    val x = A(0)._2 - A(0)._1 + 1
    val y = A(1)._2 - A(1)._1 + 1
    val z = A(2)._2 - A(2)._1 + 1
    println("%.15f".format(
      (A(0)._1 to A(0)._2)
        .map(a =>
          Math.max(0,
            A(1)._2 - Math.max(A(1)._1 - 1, a)) * Math.max(0, A(2)._2 - Math.max(A(2)._1 - 1, a)
          ).toDouble / x)
        .sum / y / z
    ))
