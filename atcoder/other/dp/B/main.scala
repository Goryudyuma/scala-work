...
object Main {
  ...
  def solve(sc: => Scanner): Unit = {
    val N, K = sc.nextInt()
    val cost = Array.fill(N)(Long.MaxValue / 6)
    cost(0) = 0
    val h = Array.fill(N)(sc.nextLong())
    for {
      now <- 0 until N
      next <- now + 1 to math.min(N - 1, now + K)
    } {
      cost(next) = math.min(cost(next), cost(now) + math.abs(h(now) - h(next)))
    }
    println(cost(N - 1))
  }
