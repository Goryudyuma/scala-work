...
object Main {
  ...
  def solve(sc: => Scanner): Unit = {
    val N, W = sc.nextInt()
    val Value = Array.fill(100001)(Long.MaxValue / 6)
    Value(0) = 0
    (0 until N).foreach { _ =>
      val w = sc.nextLong()
      val v = sc.nextInt()
      (v to 100000).foreach { i =>
        Value(100000 - i + v) = math.min(Value(100000 - i + v), Value(100000 - i) + w)
      }
    }
    println(Value.zipWithIndex.filter {
      case (v, _) => v <= W
    }.last._2)
  }
