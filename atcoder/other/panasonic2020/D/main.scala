...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(recursive(sc.nextInt()).sorted.mkString("\n"))
  }
  ...
  def recursive(N: Int, canUse: Int = 0, nowString: String = ""): Array[String] = {
    if (N == 0) Array(nowString)
    else (0 to canUse).flatMap(i => recursive(N - 1, math.max(canUse, i + 1), nowString + (i + 'a').toChar)).toArray
  }
