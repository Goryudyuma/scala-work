...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    println(
      (1 to A.sum).toArray.permutations.count(arr =>
        (0 until Math.min(A(0), A(1))).forall(x => arr(x) < arr(x + A(0)))
          && (0 until Math.min(A(1), A(2))).forall(x => arr(x + A(0)) < arr(x + A(0) + A(1)))
          && (0 until A(0) - 1).forall(x => arr(x) < arr(x + 1))
          && (0 until A(1) - 1).forall(x => arr(x + A(0)) < arr(x + A(0) + 1))
          && (0 until A(2) - 1).forall(x => arr(x + A(0) + A(1)) < arr(x + A(0) + A(1) + 1))
      )
    )
  }
  ...
  def recursive(sum: Long, ans: Array[Long]): Array[Long] = {
    if (sum == 0) ans ++ Array.fill(50 - ans.length)(49L)
    else {
      val one: Long = Math.min(Math.pow(10, 16).toLong + 800, sum)
      recursive(sum - one, ans :+ (one + 49))
    }
  }
