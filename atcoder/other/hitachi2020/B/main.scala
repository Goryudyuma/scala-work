...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A, B, M = sc.nextInt()
    val X = Array.fill(A)(sc.nextLong())
    val Y = Array.fill(B)(sc.nextLong())
    println(
      (Array.fill(M)((sc.nextInt() - 1, sc.nextInt() - 1, sc.nextLong()))
        .map(now => X(now._1) + Y(now._2) - now._3)
        :+ (X.min + Y.min)).min
    )
  }
  ...
  def recursive(S: String, now: String, set: Set[String]): Long = {
    if (S.length == 0) set.size
    else if (set.contains(now + S.head)) recursive(S.tail, now + S.head, set)
    else recursive(S.tail, "", set + (now + S.head))
  }
