...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    val A = Array.fill(N)(sc.nextLong()).sorted.reverse.zipWithIndex
    val Xsum = X + A.filter { case (a, b) => b % 2 == 0 }.map { case (a, b) => a }.sum
    val Ysum = Y + A.filter { case (a, b) => b % 2 == 1 }.map { case (a, b) => a }.sum
    println(if (Xsum == Ysum) "Draw" else if (Xsum > Ysum) "Takahashi" else "Aoki")
  }
