...
object Main {
  def solve(sc: => Scanner): Unit = {
    val ans = recursive(sc.nextLong(), 2, sc.nextLong())
    if (ans.head == -1) println(-1) else println(ans.sorted.mkString(" "))
  }
  ...
  def recursive(input: Long, maxV: Long, num: Long): List[Long] = {
    if (input == 1) List[Long](-1) else {
      if (num == 1) List[Long](input) else {
        if (input % maxV == 0) recursive(input / maxV, maxV, num - 1) :+ maxV else recursive(input, maxV + 1, num)
      }
    }
  }
