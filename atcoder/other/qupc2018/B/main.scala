...
object Main {
  def solve(sc: => Scanner): Unit = {
    (0 until sc.nextInt()).foreach { _ =>
      ...
      val sum = A * 100 + B * 10 + C
      val sum2: Long = sum / 2
      val sum3: Long = sum2 - math.min(sum2 / 100, A).toLong * 100
      val sum4: Long = sum3 - math.min(sum3 / 10, B).toLong * 10
      val sum5: Long = sum4 - math.min(sum4, C).toLong
      println(if (sum % 2 != 1 && sum5 == 0) ("Yes") else ("No"))
    }
  }
