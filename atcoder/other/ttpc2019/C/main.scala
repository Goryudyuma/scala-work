...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    println(
      if (A.contains(-1)) {
        val t = if (A.exists(l => l != -1)) A.filter(l => l != -1).reduce((x, y) => x ^ y) ^ X else X
        if (A.count(l => l == -1) == 1 && t <= X)
          ((A.takeWhile(l => l != -1) :+ t) ++ A.dropWhile(l => l != -1).drop(1).map(l => if (l == -1) 0L else l)).mkString(" ")
        else if (A.count(l => l == -1) > 1) {
          val t1 = t & X
          val t2 = t ^ t1
          if (t1 <= X && t2 <= X) {
            (((A.takeWhile(l => l != -1) :+ t1) ++ A.dropWhile(l => l != -1).drop(1).takeWhile(l => l != -1) :+ t2)
              ++ A.dropWhile(l => l != -1).drop(1).dropWhile(l => l != -1).drop(1).map(l => if (l == -1) 0L else l)).mkString(" ")
          } else -1
        }
        else -1
      } else if (!A.contains(-1) && A.reduce((x, y) => x ^ y) == X) {
        A.mkString(" ")
      } else -1
    )
  }
