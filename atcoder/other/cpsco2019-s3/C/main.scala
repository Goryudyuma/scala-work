...
object Main {
  ...
  def solve(sc: => Scanner): Unit = {
    val ints = Array.fill(2000000)(0)
    Array.fill(sc.nextInt())((sc.nextInt(), sc.nextInt())).foreach(x => {
      ints(x._1) += 1
      ints(x._2) -= 1
    })
    val imos = ints.scanLeft(0)((x, y) => x + y)
    println(imos.sliding(2).count(x => x(0) * x(1) == 0 && x(0) != x(1)) / 2)
  }
