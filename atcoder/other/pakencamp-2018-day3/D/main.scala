...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    val R, L = Array.fill(N)(0L)
    (0 until N - 1).map(i => {
      R(i + 1) = math.max(0, R(i) + A(i))
      L(N - i - 2) = math.max(0, L(N - i - 1) + A(N - i - 2))
    })
    (0 until N).foreach(i => println(math.max(R(i), L(i))))
  }
  ...
  def recursive(N1: Long, N2: Long, M1: Long, M2: Long): Long = {
    println(N1, N2, M1, M2)
    if (N1 + N2 == 0 || M1 + M2 == 0) (-1) else (recursive(math.max(0, M1 - math.max(0, N1 + N2 - M2) / 2), math.max(0, M2 - N1 - N2) + math.max(0, N1 + N2 - M2) % 2, N1, N2) + 1)
  }
