...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    println((0 until N).map(i => (0 until N).map(j => {
      val A = math.abs(i * 1.0 / N - 0.5)
      val B = math.abs((i + 1) * 1.0 / N - 0.5)
      val C = math.abs(j * 1.0 / N - 0.5)
      val D = math.abs((j + 1) * 1.0 / N - 0.5)
      if (A + C <= 0.5 && A + D <= 0.5 && B + C <= 0.5 && B + D <= 0.5) (1) else (0)
    }).sum).sum)
  }
