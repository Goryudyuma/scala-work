...
object Main {
  ...
  def solve(sc: => Scanner): Unit = {
    val input = Array.fill(sc.nextInt())(sc.next())
    val AB = input.count(S => S.head == 'B' && S.last == 'A')
    val A = input.count(S => S.last == 'A')
    val B = input.count(S => S.head == 'B')

    println(input.map(s => s.sliding(2).count(ss => ss.equals("AB"))).sum
      + (if (A == AB && B == AB && AB > 0) AB - 1 else Math.min(A, B))
    )
  }
