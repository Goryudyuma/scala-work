...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    val A = Array.fill(N)(sc.nextLong()).groupBy(identity).map(x => (x._1, x._2.length))
    println(
      if (A.size == 3 && A.keys.reduceLeft((x, y) => x ^ y) == 0 && A.values.groupBy(identity).size == 1 ||
        A.size == 2 && A.keys.min == 0 && A(0) * 3 == N ||
        A.size == 1 && A.keys.min == 0) "Yes" else "No"
    )
  }
