...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, Q = sc.nextInt()
    val E = Array.fill(N + 2)(0L)
    sc.next().sliding(2).zipWithIndex.foreach { case (s, i) => if (s == "AC") E(i + 1) += 1 }
    (1 to N + 1).foreach(i => E(i) += E(i - 1))

    (0 until Q).foreach { _ =>
      println(-E(sc.nextInt() - 1) + E(sc.nextInt() - 1))
    }
  }
