...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    val ans = (1 to N).flatMap(i => (i + 1 to N).map(j => {
      Math.min(j - i, Math.abs(i - X) + Math.abs(j - Y) + 1)
    }))
      .groupBy(identity)
    println((1 until N).map(i => ans.get(i).map(x => x.length).getOrElse(0)).mkString("\n"))
  }
  ...
  def recursive(S: Long, T: Long, ans: Long = 0): Long = {
    if (S <= T) ans
    else recursive(S, T * 4, ans + 1)
  }
