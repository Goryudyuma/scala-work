...
object Main {
  def solve(sc: => Scanner): Unit = {
    println((0 to sc.nextInt()).filter(i => i % 2 == 1).filter(i => Util.getPermutation(1).take(i).filter(x => i % x == 0).size == 8).size)
  }

  def recursive(sc: => Scanner): Array[String] = {
    val N = sc.nextInt()
    if (N != 0) {
      (Array.fill(N)((sc.next(), (sc.nextLong() * 3 + sc.nextLong() * 0 + sc.nextLong()))).
        sortWith((X, Y) => X._2 > Y._2).
        map(X => X._1 + "," + X._2).
        mkString("\n")).toString +: recursive(sc)
    } else (Array.fill(0)(""))
  }
