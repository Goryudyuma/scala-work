...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    val ans =
      N -
        Array.fill(N)(sc.nextLong())
          .foldLeft(0L)((x, y) => if (x + 1 == y) y else x)
    println(
      if (ans == N) -1 else ans
    )
  }
  ...
  def recursive(A: Array[Long], N: Option[Long]): Long = {
    if (A.length == 0)
      if (N.getOrElse(0) == 1) 1
      else 0
    else {
      N match {
        case Some(n) =>
          if (n == 1L) 1L << A.length
          else if (A.foldLeft(n)((x, y) => gcd(x, y)) != 1) 0
          else recursive(A.tail, Some(n)) + recursive(A.tail, Some(gcd(n, A.head)))
        case None =>
          if (A.reduceLeft((x, y) => gcd(x, y)) != 1) 0
          else recursive(A.tail, None) + recursive(A.tail, Some(A.head))
      }
    }
  }
