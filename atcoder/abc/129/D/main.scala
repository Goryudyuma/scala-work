...
object Main {
  def solve(sc: => Scanner): Unit = {
    val H, W = sc.nextInt()
    val input = Array.fill(H)(sc.next())
    val list1 =
      input
        .map(x =>
          x.reverse.scanLeft(0)((t, u) => if (u == '#') 0 else t + 1)
            .reverse
            .scanLeft(0)((t, u) => if (u == 0) 0 else Math.max(t, u))
            .tail.toArray)
    val list2 = Array.fill(W)(Array.fill(H)(' '))
    input.zipWithIndex.foreach(x => x._1.zipWithIndex.foreach(y => list2(y._2)(x._2) = y._1))
    ...
    list2.map(x =>
      x.reverse.scanLeft(0)((t, u) => if (u == '#') 0 else t + 1)
        ...
        .scanLeft(0)((t, u) => if (u == 0) 0 else Math.max(t, u))
        .tail)
      .zipWithIndex.foreach(x => x._1.zipWithIndex.foreach(y => list1(y._2)(x._2) = list1(y._2)(x._2) + y._1))

    println(Math.max(list1.map(x => x.max).max - 1, 0))
  }
