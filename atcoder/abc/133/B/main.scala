...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, D = sc.nextInt()
    val A = Array.fill(N)(Array.fill(D)(sc.nextDouble()))
    println(
      A.indices.map(i => (i + 1 until A.length).map(j => {
        val T = A(i).zip(A(j)).map { case (x, y) => (x - y) * (x - y) }.sum
        if (math.sqrt(T).toLong * math.sqrt(T).toLong == T) 1 else 0
      }).sum).sum)
  }
