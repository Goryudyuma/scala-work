...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A, B, H, M = sc.nextLong()
    val thetaX = (H * 60 + M) * 5.5 % 360
    val theta = Math.min(thetaX, 360 - thetaX)
    println(Math.sqrt(A * A + B * B - 2 * A * B * Math.cos(theta * Math.PI / 180)))
  }

  @scala.annotation.tailrec
  def recursive(N: Long, now: Long = 2, a: Long = 1, b: Long = 1): String = {
    if (now * now > N) (a + " " + (b * N))
    else if (N % now == 0) {
      if (N / now % now == 0) recursive(N / now / now, now, a * now, b)
      else recursive(N / now, now, a, b * now)
    } else recursive(N, now + 1, a, b)
  }
