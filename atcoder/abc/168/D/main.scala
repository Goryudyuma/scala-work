...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, M = sc.nextInt()
    val a = Array.fill(N)(Vector[Int]())
    Array.fill(M)((sc.nextInt() - 1, sc.nextInt() - 1))
      .foreach(x => {
        if (x._2 != 0) a(x._1) :+= x._2
        if (x._1 != 0) a(x._2) :+= x._1
      })
    val ans: Array[Option[Int]] = Array.fill(N - 1)(None: Option[Int])
    val que = new mutable.Queue[Int]()
    que.enqueue(0)
    while (que.nonEmpty) {
      val now = que.dequeue()
      a(now).foreach(i => if (ans(i - 1).isEmpty) {
        ans(i - 1) = Some(now + 1)
        que.enqueue(i)
      })
    }
    println("Yes")
    println(ans.map(_.get).mkString("\n"))
  }
