...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, M = sc.nextInt()
    val A = Array.fill(M)((sc.nextLong(), sc.nextLong())).sortWith((a, b) => a._2 < b._2)
    println(recursive(0, A, 0))
  }

  def recursive(now: Long, A: => Array[(Long, Long)], idx: Int): Long = {
    if (A.length == idx) (0) else if (A(idx)._1 < now && now <= A(idx)._2) (recursive(now, A, idx + 1)) else (1 + recursive(A(idx)._2, A, idx + 1))
  }
