...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(if (recursive(sc.next, sc.next)) ("Yes") else ("No"))
  }

  def recursive(s1: String, s2: String, num: Long = 0): Boolean = {
    if (s1.length == num) (false) else if (s1 == s2) (true) else (recursive(s1, s2.tail + s2.head, num + 1))
  }
