...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    val A = Array.fill(N)(Array.fill(sc.nextInt())((sc.nextInt() - 1, sc.nextInt())))
    println(
      ((0L until 1L << N).toArray.filter(now => {
        (0 until N).forall(i => !A(i).exists(x => if ((now >> i) % 2 == 1) (now >> x._1) % 2 != x._2 else false))
      }) :+ 0L).map(x => java.lang.Long.bitCount(x)).max
    )
  }
  ...
  def recursive(a: Int, A: Array[Long], sc: => Scanner): Array[Long] = {
    if (a == 1) A :+ 0L else {
      val C, S, F = sc.nextLong()
      recursive(a - 1, (A.map(i => Math.max(S, i)) :+ S).map(i => (i + F - 1) / F * F).map(i => i + C), sc)
    }
  }
