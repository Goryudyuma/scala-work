...
object Main {
  ...
  def solve(sc: => Scanner): Unit = println(recursive2(Set[Long](), recursive(sc.nextLong())) + 1)
  ...
  def recursive(X: Long): Stream[Long] = {
    Stream.cons(X, recursive(if (X % 2 == 0) X / 2 else X * 3 + 1))
  }
