...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    val B = A.zipWithIndex.map(x => x._2 - x._1).groupMapReduce(identity)(_ => 1)((a, _) => a + 1)
    val C = A.zipWithIndex.map(x => x._1 + x._2)
    println(C.map(x => B.getOrElse(x, 0).toLong).sum)
  }
