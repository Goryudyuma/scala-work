...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    println(if ((1 to 9).exists(i => (1 to 9).exists(j => i * j == N))) "Yes" else "No")
  }

  def recursive(A: Long, B: Long, C: Long): Long = {
    if (A == 0) 0 else if (A % C != 0) recursive(A - A % C, B, C) + B else recursive(A / C, B, C) + B
  }
