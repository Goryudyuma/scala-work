...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, K = sc.nextInt()
    val P = Array.fill(N)(sc.nextLong()).scanLeft(0L)((a, b) => a + b)
    println(((0 until P.length - K).map(i => P(i + K) - P(i)).max + K).toDouble / 2)
  }
  ...
  def recursive(A: Array[Int]): Long = {
    if (A.length == 0) 0 else {
      val head = A.head
      val tail = A.tail
      factorial(tail.length) * tail.count(_ < head) + recursive(tail)
    }
  }
