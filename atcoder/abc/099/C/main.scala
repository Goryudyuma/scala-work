...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt
    val A = Array.fill(N + 1)(1 << 60)
    A(0) = 0
    for (i <- 0 until N) {
      if (i + 1 <= N) {
        A(i + 1) = math.min(A(i) + 1, A(i + 1))
      }
      Util.getPermutation(1).map(x => math.pow(6, x).toInt + i).takeWhile(x => x <= N).foreach(x => A(x) = math.min(A(i) + 1, A(x)))
      Util.getPermutation(1).map(x => math.pow(9, x).toInt + i).takeWhile(x => x <= N).foreach(x => A(x) = math.min(A(i) + 1, A(x)))
    }
    println(A(N))
  }
