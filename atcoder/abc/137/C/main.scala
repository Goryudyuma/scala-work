...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(Array.fill(sc.nextInt())(sc.next()).map(s => s.sorted).groupBy(s => s).map(x => x._2.length.toLong).map(i => i * (i - 1) / 2).sum)
  }
  ...
  def recursive(N: Long, A: Array[Long]): Long = {
    if (A.length == 0) 0
    else if (N >= A.sum) A.sum
    else {
      Math.max(recursive(N - A.head, A.tail.filter(i => i <= N - A.head)) + A.head, recursive(N, A.tail))
    }
  }
