...
object Main {
  def solve(sc: => Scanner): Unit = {
    val K, X = sc.nextLong()
    println((X - K + 1 to X + K - 1).mkString(" "))
  }
  ...
  def recursive(S: String): String = {
    if (S.length < "AIZUNYAN".length) S
    else if (S.take("AIDUNYAN".length).sorted == "AIDUNYAN".sorted) {
      "AIZUNYAN" + recursive(S.drop("AIZUNYAN".length))
    }
    else S.head + recursive(S.tail)
  }
