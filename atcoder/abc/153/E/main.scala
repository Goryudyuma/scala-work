...
object Main {
  def solve(sc: => Scanner): Unit = {
    val H, N = sc.nextInt()
    val DP = Array.fill(H + 10001)(Long.MaxValue / 10)
    DP(0) = 0
    (0 until N).foreach(_ => {
      val A, B = sc.nextInt()
      (0 to H + 10000 - A).foreach(i => DP(i + A) = Math.min(DP(i + A), DP(i) + B))
    })
    (0 until H + 10000).map(i => H + 10001 - i - 2).foreach(i => DP(i) = Math.min(DP(i), DP(i + 1)))
    println(DP(H))
  }
