...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    val A = Array.fill(N - 1)(sc.nextInt()).groupBy(identity).map(a => (a._1, a._2.length))
    println((1 to N).map(A.getOrElse(_, 0)).mkString("\n"))
  ...
  def recursive(input: Array[(Int, Int)], arr: Array[Int]): Array[Int] = {
    if (input.length == 0) arr
    else {
      val now = input.head
      recursive(input.tail, arr.slice(now._1, now._1 + now._2) ++ arr.take(now._1) ++ arr.drop(now._1 + now._2))
    }
  }
