...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A = sc.next().split("LR").map("R" + _ + "L")
    A(0) = A(0).tail
    A(A.length - 1) = A.last.reverse.tail.reverse

    println(
      A.flatMap(now =>
        (Array.fill(now.count(_ == 'R') - 1)(0)
          ++
          (
            if (now.count(_ == 'R') % 2 == 0)
              Array.empty :+ now.length / 2 :+ (now.length - (now.length / 2))
            else Array.empty :+ (now.length - (now.length / 2)) :+ now.length / 2
            )
          ++ Array.fill(now.count(_ == 'L') - 1)(0))
      ).mkString(" ")
    )
  }
  ...
  def recursive(A: Array[Boolean], B: Array[(Int, Long)]): Array[Long] = {
    val next = flip(A, flip(A, B))
    if (next sameElements B) B.map(_._2)
    else recursive(A, next)
  }
