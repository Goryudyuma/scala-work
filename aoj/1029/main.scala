...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, M = sc.nextInt()
    if (N != 0 || M != 0) {
      println(recursive(sc, N, M))
      solve(sc)
    }
  }

  def recursive(sc: => Scanner, N: Int, M: Int): Long = {
    ((0L +: Array.fill(N)(sc.nextLong())) ++ Array.fill(M)(sc.nextLong())).sorted.sliding(2, 1).map(a => a(1) - a(0)).max
  }
