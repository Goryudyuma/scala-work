...
object Main {
  ...
  def solve(sc: => Scanner): Unit = {
    val m, n = sc.nextInt()
    val A = Array.fill(n)((sc.nextInt(), sc.nextInt()))
    recursive((1 to m).seq, A)
  }
  ...
  def recursive(X: Seq[Int], A: Array[(Int, Int)]): Unit = {
    if (X.nonEmpty) {
      val now = X.filterNot(i => A.map(x => x._2).contains(i))
      println(now.mkString("\n"))
      recursive(X.filter(i => A.map(x => x._2).contains(i)), A.filterNot(x => now.contains(x._1)))
    }
  }
