...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N0 = sc.nextLong()
    val P = Array.fill(4)(sc.nextLong())
    val T = Array.fill(4)(sc.nextLong())

    println(
      (0L to (N0 / T(0) + 1)).map(x0 => {
        val N1 = Math.max(N0 - x0 * T(0), 0L)
        (0L to (N1 / T(1) + 1)).map(x1 => {
          val N2 = Math.max(N1 - x1 * T(1), 0L)
          (0L to (N2 / T(2) + 1)).map(x2 => {
            val N3 = Math.max(N2 - x2 * T(2),0L)
            val x3 = N3 / T(3) + (if (N3 / T(3) * T(3) == N3) 0 else 1)
  ...
  def recursive(N: Long, cost: Long, A: Array[(Long, Int)]): Option[String] = {
    if (N == 0) Option.apply("") else if (A.length == 0) Option.empty else {
      if (cost < A.head._1) recursive(N, cost, A.tail)
      else {
        val ret = recursive(N - 1, cost - A.head._1, A)
        if (ret.isDefined) Option.apply(A.head._2 + ret.get)
        else recursive(N, cost, A.tail)
      }
    }
  }
