...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A: collection.mutable.Map[String, Seq[Long]] = collection.mutable.Map[String, Seq[Long]](
      Seq.fill(sc.nextInt())((sc.next(), sc.nextLong()))
        .groupBy { case (s, _) => s }
        .map { case (key, value) => (
          key,
          value.map {
    ...
    println(
      if (Array.fill(sc.nextInt())(sc.next())
        .foldLeft((true, 0L)) { case (prev, now) =>
          if (prev._1 && A.contains(now) && A(now).nonEmpty) {
            A(now) = A(now).dropWhile(l => l <= prev._2)
            if (A(now).isEmpty) (false, -1) else {
              (true, A(now).head)
            }
          } else {
            (false, -1)
