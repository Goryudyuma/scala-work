...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    (0 until n).foreach(_ => {
      ...
      (0 to 393).foreach(i => {
        if (A(i) != -1) {
          (s to l).filter(_ + i < 394).foreach(j => A(j + i) = Math.max(A(i) + p, A(j + i)))
        }
      })
    })
    val ans = Array.fill(sc.nextInt())(A(sc.nextInt()))
    println(if (ans.contains(-1)) -1 else ans.mkString("\n"))
  }
