...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A = Array.fill(sc.nextInt())((sc.nextLong(), sc.nextLong()))
    println(
      if (A.map(x => x._1).groupBy(identity).size != A.length || A.map(x => x._2).groupBy(identity).size != A.length) -1 else {
        val B = Array.fill(A.length)(0L)
        A.foreach(x => B(x._1.toInt - 1) = x._2.toInt - 1)
  ...
  def recursive(A: Long, B: Long, X: Long): Long = {
    if (X == 0) 0 else {
      val C1 = A / X * (X / 2)
      val C2 = Math.max(A % X - X / 2 + 1, 0L)
      val C3 = C1 + C2
      val D1 = B / X * (X / 2)
      val D2 = Math.max(B % X - X / 2 + 1, 0L)
      val D3 = D1 + D2

      recursive(A, B, (if (A / X == 0 && B / X == 0) 0 else X * 2)) * 2 + Math.abs(C3 - D3) % 2
    }
  }
