...
object Main {
  def solve(sc: => Scanner): Unit = {
    println(recursive(Array.fill(sc.nextInt())(sc.nextInt()), sc.nextInt(), sc).mkString("\n"))
  }
  ...
  def recursive(list: Array[Int], index: Int, sc: => Scanner): Array[Int] = {
    if (index == 0) list else {
      val now = sc.nextInt() - 1
      val nowPosition = list(now)
      val nextPosition = nowPosition + 1
      if (!list.contains(nextPosition) && nowPosition != 2019) {
        list(now) = nextPosition
      }

      recursive(list, index - 1, sc)
    }
  }
