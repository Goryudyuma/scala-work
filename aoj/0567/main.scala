...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    val A, B = sc.nextLong()
    val Start = sc.nextDouble()
    val Arr = Array.fill(N)(sc.nextLong()).sorted.reverse
    val ans = Arr.foldLeft((Start, A)) {
      case (x, y) => if (x._1 / x._2 < y / B) ((x._1 + y, x._2 + B)) else (x)
    }
    println((ans._1 / ans._2).toLong)
  }
