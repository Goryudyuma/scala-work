...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, T = sc.nextInt()
    val ans = Array.fill(N * 2)((sc.next(), sc.next()))
      .sliding(2)
      .map(a => (a(0), a(1)))
      .filter { case (a, b) => a._2 == b._2 }
      .map { case (a, b) => ((a._1, b._1), a._2) }
      .map { case (a, b) => (time(a._2) - time(a._1), b) }
      .filter { case (a, _) => a >= T }
      .toArray

    println(ans.length)
    if (ans.nonEmpty) println(ans.map(a => (a._2 + " " + a._1)).mkString("\n"))
  }
