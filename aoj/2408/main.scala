...
object Main {
  def solve(sc: => Scanner): Unit = {
    sc.nextInt()
    val A = Array.fill(sc.nextInt())(Array.fill(sc.nextInt())(sc.nextInt()))
    val B = Array.fill(sc.nextInt())((sc.nextInt(), sc.nextInt()))
    val C = for (v1 <- A; v2 <- B) yield (v1, v2)
    val D = C.filter { case (x, (y, z)) => x.contains(y) && x.contains(z) }.flatMap { case (x, (y, z)) => Array(y, z) }
