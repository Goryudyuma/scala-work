...
object Main {
  def solve(sc: => Scanner): Unit = {
    val ans = recursive(sc, sc.nextLong() - 1, sc.nextLong(), sc.nextLong(), sc.nextLong(), sc.nextLong(), sc.nextLong())
    println(ans._3 + " " + ans._4)
    println(ans._1 + " " + ans._2)
  }

  def recursive(sc: => Scanner, N: Long, f: Long, a: Long, t: Long, x: Long, y: Long): (Double, Double, Double, Double) = {
    if (N == 0) ((-1, -1, -1, -1)) else {
      val ff, aa, tt, xx, yy = sc.nextLong()
      val next = recursive(sc, N - 1, ff, aa, tt, xx, yy)
      if (t == tt && aa != a) {
        if (t == 1) {
          if (next._1 < math.hypot(xx - x, yy - y) || (next._1 == math.hypot(xx - x, yy - y) && next._2 > ff - f)) {
            (math.hypot(xx - x, yy - y), (ff - f) * 1.0 / 60, next._3, next._4)
          } else {
            next
          }
        } else {
          if (next._3 < math.hypot(xx - x, yy - y) || (next._3 == math.hypot(xx - x, yy - y) && next._4 > ff - f)) {
            (next._1, next._2, math.hypot(xx - x, yy - y), (ff - f) * 1.0 / 60)
          } else {
            next
          }
        }
