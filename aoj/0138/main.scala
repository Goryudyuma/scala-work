...
object Main {
  def solve(sc: => Scanner): Unit = {
    val A = Array.fill(24)((sc.nextInt, sc.nextDouble())).grouped(8).map(x => x.sortWith((a, b) => a._2 < b._2)).toArray
    for (i <- 0 until 3) {
      println(A(i)(0)._1 + " " + A(i)(0)._2)
      println(A(i)(1)._1 + " " + A(i)(1)._2)
    }
    val B = A.map(x => x.zipWithIndex.filter(y => y._2 >= 2)).flatten.map(x => x._1).sortWith((a, b) => a._2 < b._2)
    println(B(0)._1 + " " + B(0)._2)
    println(B(1)._1 + " " + B(1)._2)
  }

  def recursive(sc: => Scanner, q: Long): Unit = {
  }
