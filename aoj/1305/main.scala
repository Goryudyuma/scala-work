...
object Main {
  var data: Map[String, Array[String]] = null
  var memo: Map[String, Set[String]] = null
  var ans: Set[String] = null
  ...
  def solve(sc: => Scanner): Unit = {
    ...
    if (N != 0) {
      ...
      ans = Set[String]()
      val input = Array.fill(N)(sc.next())
        .map(s => s.dropRight(1))
      data = input.map(s => s.split(":")).map(x => (x.head, x.last.split(","))).toMap

      recursive(input.head.split(":").head)
      println((ans -- input.map(s => s.split(":").head).toSet).size)
      solve(sc)
    }
  }
  ...
  def recursive(key: String): Unit = {
    if (!ans.contains(key)) {
      ...
      if (data.contains(key)) {
        data(key).foreach(k => recursive(k))
      }
    }
  }
