...
object Main {
  ...
  def solve(sc: => Scanner): Unit = {
    val N = sc.nextInt()
    val A, B = Array.fill(N)(sc.nextLong())
    val C = A.zip(B)
    val D = C.filter { case (x, _) => x == 1 }.map { case (_, y) => y }
    val E = C.filter { case (x, _) => x == 0 }.map { case (_, y) => y }
    println(if (D.length == 0 || E.length == 0) 0 else D.min + E.min)
  }
