...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, M = sc.nextInt()
    val A = Array.fill(M)((sc.nextInt() - 1, sc.nextInt() - 1)).sorted
    val B = Array.fill(N)(Set[Int]())
    A.foreach(x => B(x._1) += x._2)

    val uf = new IUnionFind(N)

    A.foreach {
      case (g, s) => if (!uf.connected(s, g)) {
        var candidate = B(s)
        var connected = Set[Int]()
        while (candidate.nonEmpty) {
          val now = candidate.head
          candidate -= now
          ...
          if (now == g) {
            ...
            if (!connected.contains(now)) {
              connected += now
              candidate ++= B(now) -- connected
            }
          }
        }
      }
