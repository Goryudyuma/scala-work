...
object Main {
  def solve(sc: => Scanner): Unit = {
    val N, M = sc.nextInt()
    val A = Array.fill(N)(sc.nextLong()).toSet
    val B = Array.fill(M)(sc.nextLong()).toSet
    val C = A & B
    val D = A | B

    println(C.size.toString ++ " " ++ D.size.toString)
    print(C.toArray.sorted.map(i => i.toString ++ "\n").mkString(""))
    print(D.toArray.sorted.map(i => i.toString ++ "\n").mkString(""))
  }
