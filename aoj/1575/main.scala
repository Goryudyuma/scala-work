...
object Main {
  def solve(sc: => Scanner): Unit = {
    val W, H = sc.nextInt()
    println(
      Array.fill(sc.nextInt())(sc.nextInt())
        .scanLeft((0, 0))((x, y) => if (y == 0) ((x._1 + 1, x._2)) else ((x._1, x._2 + 1)))
        .tail
        .count(now => {
          val xnow = now._2
          val ynow = if (now._1 < W) now._1 else if (now._1 + 1 < W + H) now._1 - 2 else now._1 - 4
          xnow == ynow
        }))
  }
