...
object Main {
  def solve(sc: => Scanner): Unit = {
    ...
    val B = Array.fill(A.size * (A.size + 1))(-1L)
    for (i <- Range(0, A.size)) (
      for (j <- Range(i, math.min(i + 8, A.size))) {
        var n = 0
        for (k <- Range(i, j + 1)) {
          n *= 10
          n += A(k)
        }
        B((A.size - j - 1) * A.size + i) = n
      })
    println(B.filter(_ >= 0).toSet.toList.sorted.zipWithIndex.filter(x => x._1 != x._2)(0)._2)
  }
